# modify the data augmentation pipeline to use no class objects
...
# NEW Functions defined here: execute augmentations based on config
# as alternatives to create_ssl_data_augment and create_data_augment

def apply_ssl_data_augment(image_list, cfg, augment):
    ops = []
    if augment:
        """Apply a set of data augmentation transformations as described in the SimCLR paper."""
        s = cfg.AUGMENTATION.STRENGTH
        image_list = random_resized_crop(image_list, target_height=cfg.IMAGE_SIZE, target_width=cfg.IMAGE_SIZE)
        if random.uniform(0,1) < 0.5:
            image_list = flip(image_list)
        if random.uniform(0,1) < 0.8:
            ib = 0.0 if cfg.DATASETS[0] == "finegym" else 0.5*s
            image_list = color_jitter(image_list, img_brightness=0.8*s, img_contrast=0.8*s, img_saturation=0.8*s, img_hue=0.2*s, img_blur=ib)
        if random.uniform(0,1) < 0.2:
            image_list = grayscale(image_list)
    else:
        image_list = uniform_crop(image_list, size=cfg.IMAGE_SIZE)
    image_list = resize(image_list, size=cfg.IMAGE_SIZE)
    image_list = color_normalization(image_list, mean=[0.485, 0.456, 0.406], stddev=[0.229, 0.224, 0.225])
    return image_list

def apply_data_augment(image_list, cfg, augment):
    print('WARNING: create_data_augment not yet implemented')
    exit(-1)